...
{     
    ...
    /**
     * Red auxiliar utilizada para conectar un equipo a otro directamente, sin usar las redes
     * punto a punto.
     */
    protected Red auxRed;
    ...
    /**
     * Asocia la interfaz con un equipo
     * @param eq
     */
    public void Conectar(Equipo eq, String propiedades)
    {
    	int i = 0;
    	auxRed = LocalizadorRedes.New("Ethernet.Ethernet", "Vred");
    	for(i = 0; i < eq.NumInterfaces(); i++)
    	{
    		if(eq.getInterfaz(i).getRed() == null){
    			eq.getInterfaz(i).Conectar(auxRed);
    			this.Conectar(auxRed);
    			break;
    		}
    	}
    	if(i==eq.NumInterfaces()){
    		String[] prop = propiedades.split(";");
    		if(prop.length == 5){
	    		Interfaz itAux = LocalizadorRedes.NewInterfaz(prop[0],prop[1],prop[2],prop[3],prop[4]);
	    		itAux.Conectar(auxRed);
	    		eq.setInterfaz(itAux);
    		}
    	}
    	//this.red = auxRed;
