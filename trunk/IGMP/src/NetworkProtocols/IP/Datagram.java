...
public class Datagram {
  ...
  public int version;         // Version de IP (4) (4 bits)
  public int ihl;             // Longitud del header en multiplos de 4 bytes (4 bits)
  ...
  public void setPayload(byte[] datos){
	  payload = datos;
  }
  ...
  public byte[] toByte()  {
    ...
    b[18] = xx[2];
    b[19] = xx[3];
    /* Código justo y necesario para cargar un paquete IGMP */
    if (payload != null)
  ...
  public boolean verifyChecksum() {
    ...
    byte[] b = this.toByte();      // Transforma el header en un arreglo de bytes
    int ck = 0;
    for(int i=0; i<lg;i=i+2) ck+=(((b[i] & 0xff) << 8) | (b[i+1] & 0xff));
    //System.out.println("ck "+ck +"   and "+ (int) (ck&0x00ffff));
    if( (ck & 0x00ffff) == 0x00ffff) return true;else return false;
  }
